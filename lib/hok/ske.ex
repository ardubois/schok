...
Hok.defmodule_rts Ske do
  ...
  def reduce(ref, initial, f) do
     ...
     size = Tuple.product(shape)
      ...
      case type do
        {:f,32} -> cas = Hok.hok_rts (fn (x,y,z) -> cas_float(x,y,z) end)
            Hok.spawn_rts(&Ske.reduce_kernel/6,{numberOfBlocks,1,1},{threadsPerBlock,1,1},[ref,result_gpu, initial, size, cas, f])

        {:f,64} -> cas = Hok.hok_rts (fn (x,y,z) -> cas_double(x,y,z) end)
            Hok.spawn_rts(&Ske.reduce_kernel/6,{numberOfBlocks,1,1},{threadsPerBlock,1,1},[ref,result_gpu, initial, size, cas, f])

        {:s,32} -> cas = Hok.hok_rts (fn (x,y,z) -> cas_int(x,y,z) end)
            Hok.spawn_rts(&Ske.reduce_kernel/6,{numberOfBlocks,1,1},{threadsPerBlock,1,1},[ref,result_gpu, initial, size, cas, f])
  ...
  def map2(t1,t2,func) do
    ...
    result_gpu = Hok.new_gnx(shape, type)
      ...
      Hok.spawn_rts(&Ske.map2_kernel/5,{numberOfBlocks,1,1},{threadsPerBlock,1,1},[t1,t2,result_gpu,size,func])
  ...
  def map_coord_2D_no_resp(d_array, f) do
    ...
    {sizex,sizey,step} =  case Hok.get_shape_gnx(d_array) do
                             {l,c} -> {l,c,1}
                             {l,c,step} -> {l,c,step}
     ...
     ###Hok.spawn_rts(&Ske.map_coord_2D_no_resp_kernel/5,{grid_cols,grid_rows,1},{block_size,block_size,1},[d_array,step,sizex,sizey,f])
     Hok.spawn_rts(&Ske.map_coord_2D_no_resp_kernel/5,{sizex,sizey,1},{1,1,1},[d_array,step,sizex,sizey,f])
       d_array
   end
  def map_coord_2D_1_para_no_resp(d_array, par1, f) do
   ...
   {sizex,sizey,step} =  case Hok.get_shape_gnx(d_array) do
    ...
    Hok.spawn_rts(&Ske.map_coord_2D_1_para_no_resp_kernel/6,{grid_cols,grid_rows,1},{block_size,block_size,1},[d_array,step,par1,sizex,sizey,f])
      d_array
  end
  ...
  def map_coord_2D_2_para_no_resp(d_array, par1, par2, f) do
   ...
   {sizex,sizey,step} =  case Hok.get_shape_gnx(d_array) do
    ...
    Hok.spawn_rts(&Ske.map_coord_2D_2_para_no_resp_kernel/7,{grid_cols,grid_rows,1},{block_size,block_size,1},[d_array,step,par1,par2,sizex,sizey,f])
      d_array
  end
  def map_2_para_no_resp(d_array,  par1, par2, f) do
      block_size =  128;
      {l,step} = Hok.get_shape_gnx(d_array)
      size = l*step
      nBlocks = floor ((size + block_size - 1) / block_size)

      Hok.spawn_rts(&Ske.map_step_2_para_no_resp_kernel/6,{nBlocks,1,1},{block_size,1,1},[d_array,step,par1,par2,l,f])
      d_array
  end
  defk map_step_2_para_no_resp_kernel(d_array,  step, par1, par2,size,f) do
        globalId  = blockDim.x * ( gridDim.x * blockIdx.y + blockIdx.x ) + threadIdx.x
