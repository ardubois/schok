...
static ERL_NIF_TERM get_gpu_array_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
  ...
  if (strcmp(type_name, "float") == 0)
  {
    ...
    float *dev_array_f = (float *)dev_array;

    size_t result_size = sizeof(float) * nrow * ncol;
    size_t data_size = sizeof(float) * nrow * ncol;
    ...
    //// MAKE CUDA CALL
    cudaMemcpy(ptr_matrix, dev_array_f, data_size, cudaMemcpyDeviceToHost);
    error_gpu = cudaGetLastError();
    if (error_gpu != cudaSuccess)
    {
      char message[200];
  ...
  {
    int *dev_array_i = (int *)dev_array;
    size_t result_size = sizeof(int) * nrow * ncol;
  ...
  {
    double *dev_array_d = (double *)dev_array;

    size_t result_size = sizeof(double) * nrow * ncol;
    size_t data_size = sizeof(double) * nrow * ncol;
    ...
    //// MAKE CUDA CALL
    cudaMemcpy(ptr_matrix, dev_array_d, data_size, cudaMemcpyDeviceToHost);
    error_gpu = cudaGetLastError();
    if (error_gpu != cudaSuccess)
    {
      char message[200];
...
static ERL_NIF_TERM create_gpu_array_nx_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
  ...
  if (strcmp(type_name, "float") == 0)
  {
    ...
    cudaMemcpy(dev_array, array, data_size, cudaMemcpyHostToDevice);
    error_gpu = cudaGetLastError();
    if (error_gpu != cudaSuccess)
    {
      char message[200];
...
static ERL_NIF_TERM load_kernel_from_lib_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
